...
class OrchestrationEngine:
    ...
    def status(self, run_id: str | None = None) -> dict[str, object]:
        ...
        if not persisted_state and not state.active_run_id:
            # An uninitialized RuntimeState contains schema defaults (for example
            # current_phase=unknown and execution_mode=mock).  Those defaults are
            # not authoritative project status.  Project the immutable contract
            # phase and live Codex availability into the read-only status view
            # without creating a runtime state file.
        ...
        payload: dict[str, object] = {
            ...
            "status_context": {
                ...
                "optional_missing_files": list(contract.missing_files) if self.contract_role == ENGINE_HOST_ROLE else [],
            },
        }
