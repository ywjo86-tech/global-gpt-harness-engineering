...
COMMAND_REGISTRY: Mapping[str, RegisteredCommand] = {
    ...
    "lv.worker": RegisteredCommand("lv.worker", ("python3", "-m", "runtime.orchestrator.worker_runner"), "WORKER"),
...
def registry_sha256(registry: Mapping[str, RegisteredCommand] = COMMAND_REGISTRY) -> str:
    return _hash({key: {"argv": list(item.argv), "stage": item.lifecycle_stage} for key, item in sorted(registry.items())})
...
def validate_action_manifest(manifest: Mapping[str, object], *, expected_project_id: str,
    ...
    if set(payload) != _PAYLOAD_FIELDS or payload.get("schema_version") != "orchestration.fixed-action.v1":
...
def run_sealed_action(manifest: Mapping[str, object], *, expected_project_id: str,
    ...
    command = validate_action_manifest(manifest, expected_project_id=expected_project_id,
                                       ...
                                       registry=registry)
    completed = executor(list(command.argv), shell=False, stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=False)
