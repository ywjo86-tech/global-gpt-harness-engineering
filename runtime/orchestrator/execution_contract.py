...
def cross_check_contract(
    ...
    if not set(approval_context.approval_refs()).issubset(set(candidate.approval_refs)):
        return result(False, CrossCheckFailureOwner.APPROVAL_RESOLUTION, "APPROVAL_MISSING_OR_STALE")
...
def activate_contract(
    ...
    *,
    ...
    if not cross_check.passed:
        raise ContractActivationError(
            "Contract Cross-Check must pass before activation",
    ...
    required_refs = set(approval_context.approval_refs())
    if not required_refs.issubset(set(candidate.approval_refs)):
        raise ContractActivationError(
    ...
    activation_projection = {
        "contract_digest": candidate.contract_digest,
        "cross_check_digest": cross_check.evidence_digest,
        "approval_refs": sorted(required_refs),
    }
    return replace(
        candidate,
...
def codex_launch_binding_digest(
    *, cli_version: str, environment_fingerprint: str, transport_schema_digest: str, run_id: str, worker_task_id: str
) -> str:
    return _digest(
        {
...
def preflight_execution_package(package: ExecutionPackage, context: PreflightContext) -> PreflightResult:
    ...
    if not set(package.exact_tool_authorization_projection).issubset(set(context.active_contract.allowed_capabilities)):
        block("tool authorization projection exceeds Contract", "TOOL_AUTHORIZATION_DRIFT")
    ...
    if package.codex_auth_readiness_ref:
