...
class LVReviewTest(unittest.TestCase):
    ...
    def _git_fixture(self, base: Path) -> tuple[Path, dict[str, object], Path]:
        ...
        # The host test runner intentionally has no pytest dependency.  Seal a
        # tiny fixture-local pytest-compatible module into the synthetic project
        # so review tests exercise their command path without reaching into a
        # sibling project's .venv.  It supports only the simple function tests
        # generated by this fixture.
        (root / "pytest.py").write_text(
            "import pathlib, runpy, sys, traceback\n"
            "targets=[a for a in sys.argv[1:] if not a.startswith('-')]\n"
            "if not targets: targets=[str(p) for p in sorted(pathlib.Path('tests').glob('test_*.py'))]\n"
            "failed=0\n"
        ...
        subprocess.run(["git", "-C", str(root), "add", "README.md", ".gitignore", "pytest.py"], check=True)
    ...
    def test_preflight_blocks_existing_result_or_attempt(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            package_contract = ({"project_id": "fixture"}, manifest_path, {})
            patches = (
                patch("runtime.orchestrator.lv_review._assert_package", return_value=package_contract),
                patch("runtime.orchestrator.lv_review._project_root_for", return_value=root),
                patch("runtime.orchestrator.lv_review._assert_canonical_binding"),
                patch("runtime.orchestrator.lv_review._assert_source_snapshot"),
            )
            with patches[0], patches[1], patches[2], patches[3], self.assertRaisesRegex(LVReviewError, "worker result already exists"):
                _preflight("fixture-run", package_root=package, result_path=result, results_root=root / "results")
            result.unlink()
            (root / "results").mkdir()
            patches = (
                patch("runtime.orchestrator.lv_review._assert_package", return_value=package_contract),
            ...
            with patches[0], patches[1], patches[2], patches[3], self.assertRaisesRegex(LVReviewError, "attempt-01"):
                _preflight("fixture-run", package_root=package, result_path=result, results_root=root / "results")
    ...
    def test_preflight_stale_or_dirty_source_is_blocked(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            package.mkdir()
            package_contract = ({"project_id": "fixture"}, manifest_path, {})
            with patch("runtime.orchestrator.lv_review._assert_package", return_value=package_contract), patch(
                "runtime.orchestrator.lv_review._project_root_for", return_value=root
            ), patch("runtime.orchestrator.lv_review._assert_canonical_binding"), patch(
                "runtime.orchestrator.lv_review._assert_source_snapshot", side_effect=LVReviewError("source snapshot mismatch")
            ), self.assertRaisesRegex(LVReviewError, "source snapshot mismatch"):
                _preflight("fixture-run", package_root=package, result_path=root / "worker.json", results_root=root / "results")
