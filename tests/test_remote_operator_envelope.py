...
from runtime.orchestrator.remote_operator_diagnostic_envelope import (
...
def _payload(**changes):
    payload = {
        ...
        "message_id": "MSG-0001", "sequence": 1,
        ...
        "operator_directive": {
            "schema_version": OPERATOR_DIRECTIVE_SCHEMA, "project_id": "P1", "run_id": "R1", "task_id": "T1", "task_execution_id": "E1",
            "current_stage": "PREPARE", "requested_next_stage": "ACTION", "required_capabilities": ["filesystem_write"],
            "state_change_required": True, "input_artifact_digests": ["a" * 64], "gate_id": "G1", "directive_id": "D1",
        },
        "directive_digest": "",
        "expected": {"continuation_state_sha256": "b" * 64, "continuation_owner_epoch": 3, "canonical_run_state_sha256": "c" * 64, "migration_id": "", "migration_transaction_sha256": "", "migration_phase": "", "qualification_evidence_sha256": "", "source_head": "e" * 40, "runtime_release_digest": "d" * 64},
...
def _diagnostic_payload(**changes):
    payload = {
        ...
        "operator_directive": {
            "schema_version": OPERATOR_DIRECTIVE_SCHEMA, "project_id": "P1", "run_id": "R1", "task_id": "T1", "task_execution_id": "E1",
            "current_stage": "ENTRY", "requested_next_stage": "PREPARE", "required_capabilities": ["read_only_host_diagnostic"],
            "state_change_required": False, "input_artifact_digests": [], "gate_id": "G1", "directive_id": "D-DIAG-1",
        },
        "directive_digest": "",
        "expected": {"continuation_state_sha256": "", "continuation_owner_epoch": 0, "canonical_run_state_sha256": "", "migration_id": "", "migration_transaction_sha256": "", "migration_phase": "", "qualification_evidence_sha256": "", "source_head": "", "runtime_release_digest": ""},
        "authorization": {"risk_envelope_ref": "RISK-1", "risk_envelope_digest": "f" * 64, "manual_action_authorization_digest": ""},
        "read_only_request": {"schema_version": REQUEST_SCHEMA, "request_id": "REQ-DIAG-1", "operation": "repo.snapshot", "root_id": "jarvis-assistant", "relative_path": "", "start_line": 0, "line_count": 0, "service_id": ""},
...
class RemoteOperatorEnvelopeTests(unittest.TestCase):
    def test_valid_envelope_round_trips_and_binds_existing_directive(self):
        ...
        self.assertIsInstance(env, RemoteOperatorEnvelopeV2); self.assertEqual(env.message_id, "MSG-0001"); self.assertEqual(env.operator_directive.directive_id, "D1"); self.assertEqual(env.directive_digest, env.operator_directive.directive_digest); self.assertEqual(env.envelope_sha256, seal_remote_envelope(env.to_dict())["envelope_sha256"]); self.assertTrue(canonical_envelope_bytes(env.to_dict()))
    ...
    def test_invalid_time_order_and_sequence_are_rejected(self):
        ...
        with self.assertRaises(RemoteOperatorEnvelopeError): validate_remote_envelope(_payload(expires_at="2026-09-20T23:59:00+00:00"),now=NOW)
    ...
    def test_v3_diagnostic_round_trip_is_typed_and_v2_stays_v2(self):
        env=validate_remote_control_envelope(_diagnostic_payload(),now=NOW); self.assertIsInstance(env,RemoteOperatorEnvelopeV3); self.assertEqual(env.read_only_request.operation,"repo.snapshot"); self.assertEqual(env.read_only_request_digest,env.read_only_request.request_digest); self.assertIsInstance(validate_remote_control_envelope(_payload(),now=NOW),RemoteOperatorEnvelopeV2)
    ...
    def test_v3_rejects_provider_model_and_absolute_relative_path(self):
        ...
        payload=_diagnostic_payload(); payload["read_only_request"]["operation"]="project.file_range"; payload["read_only_request"]["relative_path"]="/etc/passwd"; payload["read_only_request"]["start_line"]=1; payload["read_only_request"]["line_count"]=1
