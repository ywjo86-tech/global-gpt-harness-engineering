...
Write-Step "Preparing Meta Harness install folder"
...
if (Test-Path $targetPath) {
    ...
    Write-Step "Updating existing Meta Harness"
    ...
    Write-Step "Cloning Meta Harness"
    git clone --branch $Branch $RepoUrl $targetPath
}

Write-Step "Verifying Meta Harness files"
$requiredPaths = @(
    "AGENTS.md",
    ".agents",
    "docs\harness",
...
if ($missing.Count -gt 0) {
    throw "Install completed but required Meta Harness paths are missing: $($missing -join ', ')"
}

Write-Host ""
Write-Host "Meta Harness installed successfully." -ForegroundColor Green
Write-Host "Path: $targetPath"
Write-Host ""
Write-Host "Open this folder in Codex, then test with:"
Write-Host "현재 Meta Harness에서 사용 가능한 스킬과 새 프로젝트 오케스트레이터를 요약해줘. 파일 수정 없이 확인만 해줘."
Write-Host ""
